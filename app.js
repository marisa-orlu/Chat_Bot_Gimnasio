...
const flujoDespedida = addKeyword([
  "adios",
  "adiós",
  "bye",
  "salir",
...
const flujoActividades = addKeyword([
  "Actividades",
  "actividades",
  "Actividad",
  "actividad",
...
const flujoAsesoria = addKeyword([
  "plan",
  "asesoria",
  "personalizado",
]).addAnswer(
  ...
  async (ctx, { flowDynamic, fallBack }) => {
    ...
    if (ctx.body === "A" || ctx.body === "a") {
      mensaje =
        "🔥 ¡Perfecto! Te recomendamos nuestro plan de entrenamiento para pérdida de peso. 💪";
    } else if (ctx.body === "B" || ctx.body === "b") {
      mensaje =
        "💪 ¡Genial! Nuestro programa de hipertrofia es ideal para ganar músculo.";
    } else if (ctx.body === "C" || ctx.body === "c") {
      mensaje =
        "🏃‍♂️ ¡Excelente! Tenemos rutinas para mejorar tu resistencia y rendimiento.";
    } else {
...
const flujoMenu = addKeyword([
  ...
  async (ctx, { fallBack }) => {
    //Este if comprueba si es diferentes a estas palabras returna el menu y sino entra en los flujos hijos
    if (
      ![
        "informacion",
        "contacto",
